...
describe "crowdfunder sql" do
  ...
  describe "questions" do
    ...
    it 'selects the titles of all projects and their pledge amounts' do
      expect(@db.execute("SELECT projects.title, SUM(pledges.amount) FROM projects INNER JOIN pledges ON projects.id = pledges.project_id GROUP BY projects.id ORDER BY projects.title;")).to eq([["Animal shelter needs dog food", 210], ["Help me buy a guitar", 98], ["Help save birds of paradise", 170], ["I have bed bugs!", 740], ["I want to teach English in China", 200], ["Iguana needs tail operation", 1035.5], ["My book on SQL", 20], ["The next Harry Potter", 120], ["The next Inna-Gadda-Davida", 342], ["Voldement needs a body", 489]])
    end
    ...
    it 'selects the titles of all projects that have met their funding goal' do
      expect(@db.execute("SELECT projects.title, projects.funding_goal - SUM(pledges.amount) As funding FROM projects INNER JOIN pledges ON projects.id = pledges.project_id GROUP BY projects.title HAVING 0 >= funding")).to eq([["My book on SQL", 0], ["The next Inna-Gadda-Davida", -142]])
    end

    it 'selects user names and amounts of all pledges then orders them by the amount' do 
      expect(@db.execute("SELECT a.name, SUM(b.amount) as total FROM users a INNER JOIN pledges b ON a.id = b.user_id GROUP BY a.id ORDER BY total, a.name;")).to eq([["Iguana", 10], ["Swizzle", 12], ["Sirius", 19], ["Alex", 20], ["Marisa", 24], ["Voldemort", 34], ["Amanda", 40], ["Bear", 50], ["Hermione", 50], ["Rosey", 50], ["Pacha", 60], ["Sophie", 60], ["Finnebar", 70], ["Franz", 90], ["Ena", 100], ["Whale", 125.5], ["Katie", 170], ["Squid", 270], ["Albus", 470], ["Victoria", 1700]])   
    end

    it 'selects the category names, and pledge amounts of all pledges in the music category' do
      expect(@db.execute("SELECT a.category, SUM(b.amount) FROM projects a INNER JOIN pledges b ON a.id = b.project_id GROUP BY b.id HAVING a.category = 'music';")).to eq([["music", 40], ["music", 24], ["music", 34], ["music", 12], ["music", 40], ["music", 40], ["music", 20], ["music", 230]])
    end

    it 'selects the category name and the sum total of the all its pledges for the book category' do
      expect(@db.execute("SELECT a.category, SUM(b.amount) FROM projects a INNER JOIN pledges b ON a.id = b.project_id WHERE a.category = 'books';")).to eq([["books", 140]])
    end
  end
end
